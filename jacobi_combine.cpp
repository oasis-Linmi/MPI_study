...
int main (int argc, char **argv)
{
    ...
    for(int n = 1;n <= steps;n++){
        ...
        if(myid == 0){
            ...
            MPI_Sendrecv(&a[mysize+1][1], totalsize, MPI_DOUBLE_PRECISION, myid+1, 10, &a[1][1], totalsize, MPI_DOUBLE_PRECISION, myid-1, 10, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }

        //从下至上平移数据
        if(myid == 0){
            MPI_Recv(&a[mysize+2][1], totalsize, MPI_DOUBLE_PRECISION, myid+1, 10, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }else if(myid == 3){
            MPI_Send(&a[2][1], totalsize, MPI_DOUBLE_PRECISION, myid-1, 10, MPI_COMM_WORLD);
        }else{
            MPI_Sendrecv(&a[2][1], totalsize, MPI_DOUBLE_PRECISION, myid-1, 10, &a[mysize+2][1], totalsize, MPI_DOUBLE_PRECISION, myid+1, 10, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
        begin_row = 2;
        end_row = mysize + 1;
        if(myid == 0){
