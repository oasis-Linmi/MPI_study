...
int main(int argc, char **argv){
   ...
   //Scatter receive buffer
    ...
    for(int n = 1;n <= steps;n++){
        ...
        //compute procession(exclude the edge of the mutrix)
        for(int i = begin_row+1; i <= end_row-1; i++){
            for(int j = begin_column+1; j <= end_column-1; j++){
        ...
        //compute procession(the edge of the mutrix)
        //The up row
        for(int i = begin_column;i <= end_column;i++){
            c[begin_row][i] = (b[begin_row][i+1] + b[begin_row][i-1] + b[begin_row-1][i] + b[begin_row+1][i]) * 0.25;
        }
        //The bottle row
        for(int i = begin_column;i <= end_column;i++){
            c[end_row][i] = (b[end_row][i+1] + b[end_row][i-1] + b[end_row-1][i] + b[end_row+1][i]) * 0.25;
        }
        //The left column
        for(int i = begin_row;i <= end_row;i++){
            c[i][begin_column] =(b[i][begin_column+1] + b[i][begin_column-1] + b[i-1][begin_column] + b[i+1][begin_column]) * 0.25;
        }
        //The right column
        for(int i = begin_row;i <= end_row;i++){
            c[i][end_column] =(b[i][end_column+1] + b[i][end_column-1] + b[i-1][end_column] + b[i+1][end_column]) * 0.25;
        }
        //copy to array b
        for(int i = begin_row; i <= end_row;i++){
